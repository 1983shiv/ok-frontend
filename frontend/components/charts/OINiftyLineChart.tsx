...
const OINiftyLineChart: React.FC = () => {
  ...
  const data = coiAnalysisData.timeSeriesData.map(item => ({
    ...
    totalCallOI: item.totalCallOI / 1000, // Convert to thousands
    totalPutOI: item.totalPutOI / 1000,
  }));
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.dataKey === 'niftyFuture' 
                ...
                : entry.dataKey === 'totalCallOI'
                ? `Call OI: ${entry.value.toFixed(1)}K`
                : `Put OI: ${entry.value.toFixed(1)}K`
              }
            </p>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-4 text-sm">          <div className="flex items-center">
          ...
          <div className="flex items-center">
            ...
            ></div>            <span style={{ color: theme.colors.text, opacity: 0.8 }}>Call OI</span>
          </div>
          <div className="flex items-center">
            <div 
              className="w-3 h-3 rounded mr-2"
            ...
            ></div>
            <span style={{ color: theme.colors.text, opacity: 0.8 }}>Put OI</span>
          </div>
        </div>
      </div>
...
export default OINiftyLineChart;
