...
interface OILooserBarChartProps {
  interval: '15Min' | '60Min' | 'Day';
  title: string;
}

const OILooserBarChart: React.FC<OILooserBarChartProps> = ({ interval, title }) => {
  const { oiGainerLooserData } = useSelector((state: RootState) => state.chart);
  const { theme } = useTheme();

  const data = oiGainerLooserData.oiLooser[interval].map((item) => ({
    name: item.displayText,
    value: Math.abs(item.oiChange), // Convert to positive for chart display
    ...
    <div className="h-full">
      <div className="mb-4">
        ...
        <p className="text-sm opacity-70" style={{ color: theme.colors.text }}>
          Top 5 OI Losers - {interval === '15Min' ? '15 Min' : interval === '60Min' ? '60 Min' : 'Day'}
        </p>
      </div>
      <div className="h-80">
...
export default OILooserBarChart;
